...
#   scripts/pack.sh lint              → lint exactly what ships
#
...
if [ "$MODE" = "lint" ]; then
  # Exits 1 on the known Firefox background.service_worker error (Chrome-first,
  # ignore it) — that's expected, not a packaging problem.
